 ...
 */
const promptBuilder_1 = require("../src/utils/promptBuilder");
describe('StepWise Variables Replacement', () => {
    describe('replaceVariables 函数', () => {
        it('应该替换 $name 变量', () => {
            const prompt = 'Hello $name, your age is $age';
            const data = { name: 'John', age: 25 };
            const result = (0, promptBuilder_1.replaceVariables)(prompt, data);
        ...
        it('支持对象类型的变量', () => {
            ...
            const result = (0, promptBuilder_1.replaceVariables)(prompt, { data: objValue });
            expect(result).toContain(JSON.stringify(objValue, null, 2));
        });
        it('空 data 时应该返回原始提示词', () => {
            const prompt = 'No variables here';
            const result = (0, promptBuilder_1.replaceVariables)(prompt, {});
            expect(result).toBe('No variables here');
        });
    });
});
//# sourceMappingURL=StepWise.variables.test.js.map