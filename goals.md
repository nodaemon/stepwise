...
Search Light Agent Team 是一个高效构建具备稳定完成复杂代码任务智能体的工具。他支持定制复杂任务的处理流程，然后将流程中各个步骤分配给子Agent执行，从而实现负责任务的稳定处理。
...
1. **日志记录**：详细记录所有任务的完整提示词、执行过程、执行结果、执行时间等等信息，方便任务执行出错时进行调试
   日志放在[任务目录]/logs下，对于 普通任务-普通任务-收集任务-处理任务+收集-处理任务... 多个任务，按任务层级命名日志目录
   例如： 1_task  2_task  3_collect 3_1_process_and_collect 3_1_1 process 3_1_2 process ...
...
4. **调试模式**：调试模式打开后，所有收集任务体诗词不变，执行完成以后，在接口返回给用户前只返回第一个数据，方便快速调试

---

##  任务执行信息目录结构

多次任务执行，执行信息统一存放在执行时的cwd目录下的agent_team_exec_infos中，例如
```
